...
- Use a full clone, without `--filter=blob:none`, for the umbrella and every submodule: they all use `cargo ratchet`, which reads the repository through libgit2 and fails with `object not found` in a partial clone. Initialize only the tool submodules required by the task.
